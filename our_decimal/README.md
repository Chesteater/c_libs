...
| ------ | ------ |------------------------------------------------------------------------------------|
| Сложение | + | int our_add(our_decimal value_1, our_decimal value_2, our_decimal *result)         |
| Вычитание | - | int our_sub(our_decimal value_1, our_decimal value_2, our_decimal *result)         |
| Умножение | * | int our_mul(our_decimal value_1, our_decimal value_2, our_decimal *result) | 
| Деление | / | int our_div(our_decimal value_1, our_decimal value_2, our_decimal *result) |
...
| Название оператора | Оператор  | Функция | 
| ------ | ------ | ------ |
| Меньше  | < | int our_is_less(our_decimal, our_decimal) |
| Меньше или равно | <= | int our_is_less_or_equal(our_decimal, our_decimal) | 
| Больше | \> |  int our_is_greater(our_decimal, our_decimal) |
| Больше или равно | \>= | int our_is_greater_or_equal(our_decimal, our_decimal) | 
| Равно | == |  int our_is_equal(our_decimal, our_decimal) |
| Не равно | != |  int our_is_not_equal(our_decimal, our_decimal) |
...
| ------ | ------ |
| Из int | int our_from_int_to_decimal(int src, our_decimal *dst) |
| Из float  | int our_from_float_to_decimal(float src, our_decimal *dst) |
| В int  | int our_from_decimal_to_int(our_decimal src, int *dst) |
| В float  | int our_from_decimal_to_float(our_decimal src, float *dst) |
...
| Описание | Функция                                                  | 
| ------ |----------------------------------------------------------|
| Округляет указанное Decimal число до ближайшего целого числа в сторону отрицательной бесконечности. | int our_floor(our_decimal value, our_decimal *result)    |	
| Округляет Decimal до ближайшего целого числа. | int our_round(our_decimal value, our_decimal *result)    |
| Возвращает целые цифры указанного Decimal числа; любые дробные цифры отбрасываются, включая конечные нули. | int our_truncate(our_decimal value, our_decimal *result) |
| Возвращает результат умножения указанного Decimal на -1. | int our_negate(our_decimal value, our_decimal *result)   |
